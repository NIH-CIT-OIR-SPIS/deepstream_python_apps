...
def main(args):
    ...
    for i in range(number_sources):
        ...
        uri_name = args[i]
        if uri_name.find("argus://") == 0:

            source_bin = Gst.ElementFactory.make("nvarguscamerasrc", "nv-argus-camera-source")
            if not source_bin:
                sys.stderr.write(" Unable to create Source \n")
            sensor_id = int(0)
            source_bin.set_property("sensor-id", sensor_id)
            try:
                source_bin.set_property("bufapi-version", 1)
            except Exception:
                pass
            ...
            camera_props = "video/x-raw(memory:NVMM), width=(int){}, height=(int){}, framerate=(fraction){}/1, format=(string)NV12".format(1280, 720, 30)
            ...
            source_bin.link(caps_nvarguscamerasrc)
